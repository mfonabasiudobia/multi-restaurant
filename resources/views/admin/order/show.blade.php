                    ...
                    <div class="d-flex justify-content-between flex-wrap gap-2">
                        ...
                        @if($paymentProofUrl)
                            <div  class="border rounded p-2">
                                <h5>{{ __('Payment Proof') }}</h5>
                    
                                
                                    <div >
                                        <a href="{{ $paymentProofUrl }}" target="_blank" class="btn btn-info">
                                            <i class="fa-solid fa-file-pdf"></i> 
                                        </a>
                                        <a href="{{ route('admin.order.downloadPaymentProof', $order->id) }}" class="btn btn-info">
                                ...
                                @foreach ($orderStatus as $satus)
                                    <li>
                                        <a class="dropdown-item"
                                            href="{{ route('admin.order.status.change', $order->id) }}?status={{ $satus->value }}">
                                            {{ __($satus->value) }}
                                        </a>
                                    </li>
                                @endforeach
...
{{-- 
            ...
            <div class="card mt-3">
                <div class="card-body">
                    <h5>{{ __('Logistics Info') }}</h5>

                    @foreach($order->products as $product)
                    <div class="mt-3 d-flex flex-column gap-2 border-bottom pb-3">
                        <div class="fw-bold">{{ $product->name }}</div>
                        <div>
                            <span>{{ __('Bag Number') }}: </span>
                            <span class="fw-medium">{{ $product->bag_number ?? 'N/A' }}</span>
                        </div>
                        <div>
                            <span>{{ __('Row') }}: </span>
                            <span class="fw-medium">{{ $product->row ?? 'N/A' }}</span>
                        </div>
                        <div>
                            <span>{{ __('Location') }}: </span>
                            <span class="fw-medium">{{ $product->location ?? 'N/A' }}</span>
                        </div>
                    </div>
                    @endforeach
                </div>
