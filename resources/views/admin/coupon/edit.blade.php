                    ...
                    <div class="card-body">
                        <div class="row">
                                    ...
                                    <x-select name="shops[]" label="{{ __('Select shops') }}"
                                        ...
                                        @foreach ($shops as $shop)
                                            <option value="{{ $shop->id }}"
                                                @if (in_array($shop->id, $coupon->shops->pluck('id')->toArray())) selected @endif>
                                                {{ $shop->name }}
                                            </option>
                                        @endforeach
                                    </x-select>
                            ...
                            <div class="col-12 col-md-6 mb-3">
                                <x-input name="code" type="text" placeholder="Coupon code" label="Voucher Code"
                                    :value="$coupon->code" required="true" />
                            </div>
                                ...
                                <x-select name="discount_type" label="Discount Type" required="true">
                                    @foreach ($discountTypes as $type)
                                        <option value="{{ $type->value }}"
                                            @if ($coupon->type->value == $type->value) selected @endif>
                                            {{ __($type->value) }}
                                        </option>
                                    @endforeach
                                </x-select>
                            ...
                            <div class="col-12 col-md-6">
                                <x-input name="discount" label="Discount" type="text" placeholder="Discount"
                                    :value="$coupon->discount" onlyNumber="true" required="true" />
                            </div>

                            <div class="col-12 col-md-6 mb-3">
                                <x-input name="min_order_amount" type="text" placeholder="Minimum Order Amount"
                                    :value="$coupon->min_amount" label="Minimum Order Amount" onlyNumber="true" required="true" />
                            </div>

                            <div class="col-12 col-md-6 mb-3">
                                <x-input name="limit_for_user" label="Limit For Single User" type="text"
                                    placeholder="exm: 5" onlyNumber="true" :value="$coupon->limit_for_user" />
                            </div>

                            <div class="col-12 col-md-6 mb-3">
                                <x-input name="max_discount_amount" label="Maximum Discount Amount" type="text"
                                    placeholder="exm: $300" onlyNumber="true" :value="$coupon->max_discount_amount" />
                            </div>
                                ...
                                <div class="row">
                                    <div class="col-12 col-md-6 mb-3">
                                        <x-input type="text" id="datepicker" label="Start Date" name="start_date"
                                            :value="Carbon\Carbon::parse($coupon->started_at)->format('m/d/Y')" required="true" placeholder="mm/dd/yyyy" />
                                    </div>
                                    <div class="col-12 col-md-6 mb-3">
                                        <x-input type="time" id="timepicker" label="Start Time" name="start_time"
                                            :value="Carbon\Carbon::parse($coupon->started_at)->format('H:i')" required="true" />
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="row">
                                    <div class="col-12 col-md-6">
                                        <x-input type="text" id="datepicker2" label="Expired Date" name="expired_date"
                                            :value="Carbon\Carbon::parse($coupon->expired_at)->format('m/d/Y')" required="true" placeholder="mm/dd/yyyy" />
                                    </div>
                                    <div class="col-12 col-md-6">
                                        <x-input type="time" id="timepicker2" label="Expired Time" name="expired_time"
                                            :value="Carbon\Carbon::parse($coupon->expired_at)->format('H:i')" required="true" />
                                    </div>
                                </div>
                            </div>
                        </div>
