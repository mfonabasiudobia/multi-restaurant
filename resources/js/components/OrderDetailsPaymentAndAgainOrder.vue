...
const paymentProcessing = () => {
    ...
    // Log the order details to verify the payable amount
    console.log('Processing payment for order:', props.order);
    console.log('Order payable amount:', props.order.payable_amount);
    console.log('Order delivery charge:', props.order.delivery_charge);
    ...
    axios.get('/order-payment/' + props.order.id + '/' + paymentGateway.value, {
        ...
        if (response.data.data.order_payment_url) {
            // Store order ID in localStorage for payment tracking
            localStorage.setItem('pendingPaymentOrderId', props.order.id);
...
const openPaymentPopupWindow = (url) => {
    ...
    function trackURLChanges() {
        try {
            ...
            console.error('Error tracking payment window:', error);
            clearInterval(intervalID);
        }
    }
