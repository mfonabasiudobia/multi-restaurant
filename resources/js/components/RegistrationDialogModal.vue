...
// Add this watch for countries prop changes
watch(() => props.countries, (newCountries) => {
    if (newCountries && newCountries.length > 0) {
        const romania = newCountries.find(c => c.name === 'Romania');
        if (romania) {
            registerFormData.value.country = romania.name;
            registerFormData.value.phone_code = romania.phone_code;
        }
    }
}, { immediate: true });

// Keep the existing watch for country changes
watch(() => registerFormData.value.country, (newCountry) => {
    if (newCountry) {
        const selectedCountry = props.countries.find(country => country.name === newCountry);
        if (selectedCountry) {
            registerFormData.value.phone_code = selectedCountry.phone_code;
        }
    }
});
...
const registerFormSubmit = () => {
    ...
    axios.post('/registration', registerFormData.value).then((response) => {
        // Don't set token or user yet - wait for verification
        toast({
            ...registerMessage,
            props: {
        ...
        const phoneNumber = registerFormData.value.phone;
        const phoneCode = registerFormData.value.phone_code;
...
const verifyOTP = () => {
    ...
    axios.post('/verify-otp-register', {
        ...
        // Only set token and user after successful verification
