...
watch(() => forgetPassword.value.country, (newCountry) => {
    if (newCountry) {
        const selectedCountry = props.countries.find(country => country.name === newCountry);
        if (selectedCountry) {
            forgetPassword.value.phone_code = selectedCountry.phone_code;
        }
    }
});
...
watch(() => props.countries, (newCountries) => {
    if (newCountries.length > 0) {
        ...
        if (romania) {
            forgetPassword.value.country = romania.name;
            forgetPassword.value.phone_code = romania.phone_code;
        }
    }
}, { immediate: true });
