...
class OrderRepository extends Repository
{
    ...
    {
        // Use the payable amount from the frontend if provided
        $totalPayableAmount = $request->has('payable_amount') ? $request->payable_amount : 0;
        \Log::info('Cart items: ' . json_encode($carts));
        \Log::info('Using payable amount from request:', [$totalPayableAmount]);

        // First verify stock availability for all products
        foreach ($carts as $cart) {
            if ($cart->product->quantity < $cart->quantity) {
                throw new \Exception("Product {$cart->product->name} is out of stock");
            }
        }
        ...
        foreach ($shopProducts as $shopId => $cartProducts) {
            ...
            foreach ($giftProcucts as $giftProduct) {
                ...
                // Decrement stock and check if we need to remove from other carts
                $giftProduct->product->decrement('quantity', $giftProduct->quantity);
                if ($giftProduct->product->quantity <= 1) {
                    // Remove from other users' carts
                    $affectedCustomers = Cart::where('product_id', $giftProduct->product->id)
                        ->where('customer_id', '!=', auth()->user()->customer->id)
                        ->get();

                    foreach ($affectedCustomers as $affectedCart) {
                        NotificationRepository::storeByRequest([
                            'user_id' => $affectedCart->customer->user_id,
                            'title' => 'Product Removed from Cart',
                            'message' => "'{$giftProduct->product->name}' has been removed from your cart due to stock unavailability.",
                            'type' => 'cart_update',
                            'icon' => 'bi-cart-x-fill',
                            'url' => '/cart'
                        ]);
                    ...
                    Cart::where('product_id', $giftProduct->product->id)
                        ->where('customer_id', '!=', auth()->user()->customer->id)
                        ->delete();
                }
            ...
            foreach ($newCartProducts as $cart) {
                ...
                if ($cart->product->quantity <= 1) {
                    // Remove from other users' carts
                    $affectedCustomers = Cart::where('product_id', $cart->product->id)
                        ->where('customer_id', '!=', auth()->user()->customer->id)
                        ->get();
                    ...
                    foreach ($affectedCustomers as $affectedCart) {
                        NotificationRepository::storeByRequest([
                            'user_id' => $affectedCart->customer->user_id,
                            'title' => 'Product Removed from Cart',
                            'message' => "'{$cart->product->name}' has been removed from your cart due to stock unavailability.",
                            'type' => 'cart_update',
                            'icon' => 'bi-cart-x-fill',
                            'url' => '/cart'
                        ]);
                    ...
                    Cart::where('product_id', $cart->product->id)
                        ->where('customer_id', '!=', auth()->user()->customer->id)
                        ->delete();
                }
    ...
    private static function createNewOrder($request, $shop, $paymentMethod, $getCartAmounts)
    {
        ...
        // Use delivery charge from request if provided, otherwise use calculated value
        $deliveryCharge = $request->has('delivery_charge') ? $request->delivery_charge : $getCartAmounts['deliveryCharge'];

        // Use payable amount from request if provided, otherwise use calculated value
        $payableAmount = $request->has('payable_amount') ? $request->payable_amount : $getCartAmounts['payableAmount'];

        \Log::info('Creating order with delivery charge:', [$deliveryCharge]);
        \Log::info('Creating order with payable amount:', [$payableAmount]);
    ...
    private static function getCartWiseAmounts(Shop $shop, $products, $couponCode = null): array
    {
        ...
        if (is_countable($products)) {
            foreach ($products as $cart) {
                ...
                if ($cart->size_id) {
                    $size = $product->sizes()->where('id', $cart->size_id)->first();
                    if ($size) {
